<?php
/**
 * pages/deliveries.php — Delivery tracking dashboard fragment
 * Shows all panels grouped by delivery date and project
 */
require_once __DIR__ . '/../functions.php';

// All panels with delivery info, ordered by target date
$allPanels = get_panels();
$pcolors   = panel_status_colors();
$plabels   = panel_status_labels();

// Separate overdue / upcoming / delivered
...
<div class="panel mb-3">
  ...
  <div class="panel-body p-0">
    <?= _delivery_table($overdue, $pcolors, 'overdue') ?>
  </div>
</div>
<?php endif; ?>
...
<div class="panel mb-3">
  ...
  <div class="panel-body p-0">
    ...
    <?php else: ?>
      <?= _delivery_table($upcoming, $pcolors, 'upcoming') ?>
    <?php endif; ?>
  </div>
</div>
...
<div class="panel">
  ...
  <div class="panel-body p-0">
    ...
    <?php else: ?>
      <?= _delivery_table(array_slice($delivered, 0, 30), $pcolors, 'delivered') ?>
    <?php endif; ?>
  </div>
</div>

<?php
function _delivery_table(array $panels, array $pcolors, string $type): string {
    ob_start(); ?>
      ...
      <table class="table table-hover align-middle mb-0 data-table">
        <thead>
          <tr>
            ...
            <?php if ($type === 'overdue'): ?><th>กำหนดส่ง</th><th>ล่าช้า</th>
            <?php elseif ($type === 'upcoming'): ?><th>กำหนดส่ง</th><th>คงเหลือ</th>
            <?php else: ?><th>วันส่งจริง</th>
            <?php endif; ?>
        ...
        <tbody>
          <?php foreach ($panels as $pn):
              $es = $pn['eff_status']; ?>
            <tr onclick="SPA.go('/projects/view/<?= (int)$pn['project_id'] ?>')" style="cursor:pointer">
              ...
              <?php if ($type === 'overdue'): ?>
                <td class="small text-danger"><?= e(format_date($pn['target_delivery_date'])) ?></td>
              ...
              <?php else: ?>
                <td class="small text-success"><?= e(format_date($pn['actual_delivery_date'])) ?></td>
              <?php endif; ?>
              <td>
                <div class="progress-mini"><div class="bar" style="width:<?= (int)$pn['progress_percent'] ?>%;background:<?= $pcolors[$es] ?? '#6B7280' ?>"></div></div>
                <span class="small text-muted"><?= (int)$pn['progress_percent'] ?>%</span>
              </td>
              <td><span class="status-pill" style="background:<?= $pcolors[$es] ?? '#6B7280' ?>"><?= e(panel_status_label($es)) ?></span></td>
